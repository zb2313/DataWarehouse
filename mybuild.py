...
class MedicalGraph:
    def __init__(self):
        ...
        self.data_path = os.path.join(cur_dir, 'data/movies3.json')
        self.g = Graph("http://YourIP:YourPort/", auth=("neo4j", "test"))
        ...
        for data in open(self.data_path, encoding='utf-8'):
            ...
            if 'Director' in data_json:
                ...
                if len(tempdir) == 2:
                    ...
                    cooperate_dandd.append([dir1, dir2])
                    cooperate_dandd.append([dir2, dir1])
                for director in data_json['Director']:
                    if 'Actors' in data_json:
                        for actor in data_json['Actors']:
    ...
    def create_graphrels(self):
        ...
        self.create_relationshiptime('Movie', 'Time', has_time, 'has_time', '电影的上映时间')

    '''创建实体关联边'''

    def create_relationshiptime(self, start_node, end_node, edges, rel_type, rel_name):
        count = 0
        # 去重处理
        # set_edges = []
        # for edge in edges:
        ...
        for edge in edges:
            ...
            # return r.num
            verify = 'match(p:%s{name:"%s"}),(q:%s{name:["%s","%s","%s"]}),z=(p)-[r:%s]->(q) RETURN r.num' % (
            start_node, p, end_node, q[0],q[1],q[2], rel_type)
            r_num = self.g.run(verify)
            temp_num0 = r_num.data()
            if len(temp_num0) == 0:
                query = 'match(p:%s),(q:%s) where p.name="%s" and q.name=["%s","%s","%s"] create (p)-[rel:%s{name:"%s",num:%d}]->(q)' % (
                start_node, end_node, p, q[0],q[1],q[2], rel_type, rel_name, 1)
            else:
                query = 'match(p:%s{name:"%s"}),(q:%s{name:["%s","%s","%s"]}),z=(p)-[r:%s]->(q) set r.num=r.num+1' % (
                start_node, p, end_node, q[0],q[1],q[2], rel_type)
            ...
            # 这里高斯劳资了，MATCH (<node1-label-name>:<nade1-name>),(<node2-label-name>:<node2-name>)
            # CREATE
