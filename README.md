...
+ 在数据transform的过程中，有许多的数据并不是非常完整，比如没有上映时间，没有版本信息，版本分类比较随意等等。对于上映时间，如果没有，则选取最早的一条评论的时间作为上映时间。对于商品的Title处理不太完整，可能会带有商品版本信息，没有将电影名完全抽取出来，等等
+ 由于第二种亚马逊页面的限制，爬取到的演员数量偏少，可以考虑引入第三方数据源。在筛选电影数据中，我们选择了有imdb评分的商品，可能会疏漏一些电影。
