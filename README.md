...
- **为某些关系设置某些属性，方便查询。**在查询合作次数最多的导演和导演，合作次数最多的演员和导演，合作次数最多的演员和演员等中直接查询关系相应的合作次数进行比较后，按照要求选取top100等。查询关系为O(n)，排序为调用neo4j中的排序。
- **如果配置够好的话(或者开启虚拟内存)，为neo4j内存分配尽可能大。**在配置文件中neo4j的默认设置为dbms.memory.heap.initial_size=512m；dbms.memory.heap.max_size=512m，根据硬件配置，做合适的修改（最大堆内存越大越好，但是要小于机器的物理内存）
- **对合适的数据进行拆分，即变为多个维度**。在本次项目中，在查询业务中有针对某年，某月，某日新增电影的查询。将上映时间变为年，月，日，等维度(即list)，那么在针对时间对电影进行查询时就可以实现更高的查询效率。
- 对Hive中表的查询，与Mysql一样设置有**数据的冗余**，对某些数据的查询直接搜索冗余数据，提高性能，便于一次写入，多次查询。
- 对于Hive查询的优化，分别从**物理资源和文件格式两个方面进行优化**。对于分布式系统中的物理资源，包括物理主机数量或者是单台物理主机的计算资源，资源越多，查询的效率越高。同时，对比了Hive中存储文件的不同格式，默认的Textfile格式导入数据简单，但文件占用存储较多，查询效率一般；优化后的ORC格式导入数据较为复杂，需要从Textfile格式的表中导入，但总体占用存储仅为默认格式的三分之一，查询速度也更快，在更大数据规模的情况下查询性能还有很大潜力。
- 本文档中的分布式实验数据均来自于测试环境，实际部署服务器上后发现对于某些数据量较大的查询需要耗费50秒甚至1分钟，因为分布式系统对物理资源的需求较大，而我们使用的服务器性能较差，故查询效率低下。
